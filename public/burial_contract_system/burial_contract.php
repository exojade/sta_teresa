...
use mikehaertl\pdftk\Pdf;
    if($_SERVER["REQUEST_METHOD"] === "POST") {
		...
		if($_POST["action"] == "burial_contract_pdf"){
			...
			$discount = query("select * from transaction where contract_id = ?
							and payment_type = 'DISCOUNT'", $_POST["contract_id"]);
			$total_discount = 0;
			$discount_amount = "";
			$discount_title = "";
			if($discount != ""){
				...
				$discount_amount = to_peso($discount["amount"]);
			}
			...
			$payment_total = $total_cash + $total_guarantee + $total_discount;
            ...
            $result = $pdf->fillForm([
			  ...
			  "contract_date"    => $burial_contract["contract_date"],
			  ...
			  "contract_number" => $burial_contract["contract_id"],
			  "remarks" => $burial_contract["remarks"],
